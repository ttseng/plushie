...
function loadDefaultGestures() {
  ...
  // load default data
  model.load(`${dataDirectory}/default-gestures.json`, dataLoaded);
}
...
function addDefaultGestures() {
  ...
  console.log('add default gestures ', gestures);
...
function addNewData(id) {
  ...
  let sampleLength = 90;
  let exampleData = [accelX.slice(0, sampleLength), accelY.slice(0, sampleLength), accelZ.slice(0, sampleLength)];
  console.log('exampleData: ', exampleData);
  model.addExample(exampleData, targetGesture);
...
function runPrediction() {
  ...
  if (microbitPaired && !isTraining && accelXArr.length > num_samples) {
    ...
    model.classify([ax_data, ay_data, az_data], predictionResults);
  }
}

function predictionResults(error, result) {
  if (error) {
  ...
  currentState = result.label.toLowerCase();
  gestureLog.push(currentState);
  let confidences = result.confidencesByLabel;

  let confidence = confidences[result.label];
  ...
  updateConfidenceGestures(Object.keys(confidences));
...
function showConfidence(confidences) {
  let labels = Object.keys(confidences);
  labels.forEach(function (labelName, index) {
    let confidenceContainer = document.querySelector(`#gesture-confidence-container .confidence.${labelName}`);
    let confidence = (confidences[labelName] * 100).toFixed(0)
